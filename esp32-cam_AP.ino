...
WARRANTIES, SO SUCH EXCLUSION MAY NOT APPLY TO YOU. EXCEPT TO THE EXTENT REQUIRED BY
...
void setup() {
  ...
  config.pixel_format = PIXFORMAT_JPEG;  // parar streaming
  //config.pixel_format = PIXFORMAT_RGB565; // para detección y reconocimiento de cara
  config.grab_mode = CAMERA_GRAB_WHEN_EMPTY;
  config.fb_location = CAMERA_FB_IN_PSRAM;
  config.jpeg_quality = 12;
  config.fb_count = 1;

  // si PSRAM IC, iniciar con resolución UXGA y mayor calidad JPEG
  // para una mayor memoria intermedia preasignada.
  if (config.pixel_format == PIXFORMAT_JPEG) {
    if (psramFound()) {
      ...
      // Limitar el tamaño de la trama cuando no se dispone de PSRAM
      config.frame_size = FRAMESIZE_SVGA;
      config.fb_location = CAMERA_FB_IN_DRAM;
    }
  } else {
    // La mejor opción para la detección/reconocimiento de caras
  ...
  // los valores iniciales están invertidos verticalmente y los colores están un poco saturados
  if (s->id.PID == OV3660_PID) {
    s->set_vflip(s, 1);        // dar la vuelta ala imagen
    s->set_brightness(s, 1);   // subir un poco el brillo
    s->set_saturation(s, -2);  // bajar la saturación
  }
  // reducir el tamaño de los fotogramas para aumentar la frecuencia de imagen inicial
  if (config.pixel_format == PIXFORMAT_JPEG) {
...
// Configurar LED FLash si el pin LED está definido en camera_pins.h
  ...
  Serial.println();
  Serial.print("Camara en marcha! conectar a ESP32-CAM AP y acceder a http://");
